...
def login(request):
    ...
    logged_pass= Doctor.objects.filter(password=request.POST['password'])
    ...
    if bcrypt.checkpw(request.POST['password'].encode(), logged_user.password.encode()):
        ...
        error=True
    if error==True:
        messages.error(request,"Wrong password ", extra_tags = 'wrongpass')
        #  error=True
...
def registration(request):
    error=False
    checkd = Doctor.objects.filter(email = request.POST['email'])
    checkp = Doctor.objects.filter(password = request.POST['password'])

    
    if  checkd:
        messages.error(request,'this Doctor already exist !! ',extra_tags='adddoctor')
        error=True
    if  checkp:
        messages.error(request,'this Doctor already exist !! ',extra_tags='adddoctorp')
        error=True 
    if error==True:
        return redirect('/dashboard')
    else:
